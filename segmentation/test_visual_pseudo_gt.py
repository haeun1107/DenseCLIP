...
def _legend_strip(width, class_names, palette, max_cols=4, pad=8, bg=(35,35,35)):
    ...
    cols = min(max_cols, len(class_names))
    h = pad*2 + rows*row_h
...
def parse_args():
    ...
    parser.add_argument('--slice-index', type=int, default=None,
                        help='.nii(.gz)')
    parser.add_argument('--pseudo-dir', help='root dir of pseudo NIfTI labels (middle panel)')
    ...
    for i in range(N):
        ...
        try:
            ...
            if getattr(dataset, 'reduce_zero_label', False):
                gt = _reduce_zero_label(gt, ignore_index=args.mask_ignore)
            if gt.ndim == 2:
                Hg, Wg = gt.shape
        except Exception:
            gt = None
        ...
        if getattr(args, 'pseudo_dir', None):
            try:
                img_base = getattr(dataset, 'img_dir', None) or getattr(dataset, 'img_prefix', None)
                rel_img = None
                if img_base and img_path and osp.isabs(img_path):
                    try:
                        rel_img = osp.relpath(img_path, img_base)
                    except Exception:
                        rel_img = osp.basename(img_path)
                else:
                    rel_img = osp.basename(img_path) if img_path else f'img_{i:06d}.nii.gz'
                ...
                if ext == '.gz':
                    ...
                    ext = '.nii.gz'
                pseudo_rel = (base + '_gt' + ext).lstrip('/')

                pseudo_path = osp.join(args.pseudo_dir, pseudo_rel)
                if not osp.exists(pseudo_path):
                    alt = osp.join(args.pseudo_dir, osp.basename(pseudo_rel))
                    if osp.exists(alt):
                        pseudo_path = alt

                if osp.exists(pseudo_path):
                    pvol = nib.load(pseudo_path).get_fdata()
                    pm = np.asarray(pvol)
                    if pm.ndim == 3:
                        s = args.slice_index if args.slice_index is not None else pm.shape[-1] // 2
                        s = max(0, min(s, pm.shape[-1]-1))
                        pm = pm[..., s]
                    pseudo = pm.astype(np.uint8)
                    pseudo_used = True
            except Exception:
                pseudo = None
        ...
        if pseudo is not None and pseudo.shape != (H, W):
            pseudo = cv2.resize(pseudo.astype(np.uint8), (W, H), interpolation=cv2.INTER_NEAREST)
        ...
        pseudo_color = _colorize(pseudo, palette, ignore_index=args.mask_ignore) if pseudo is not None else np.zeros_like(img)
        ...
        if args.vis_mode == 'overlay':
            pseudo_vis = cv2.addWeighted(img, 1.0 - float(args.opacity), pseudo_color, float(args.opacity), 0)
            gt_vis     = cv2.addWeighted(img, 1.0 - float(args.opacity), gt_color,     float(args.opacity), 0)
        else:
            pseudo_vis = pseudo_color
            gt_vis     = gt_color

        if args.mask_by_gt and gt is not None:
            valid = (gt != args.mask_ignore)
            pv2 = pseudo_color.copy()
            pv2[~valid] = 0
            pseudo_vis = cv2.addWeighted(img, 1.0 - float(args.opacity), pv2, float(args.opacity), 0) \
                         if args.vis_mode == 'overlay' else pv2

        if args.outline:
            if pseudo is not None:
                pseudo_vis = _draw_class_contours(pseudo_vis, pseudo, palette, thickness=2, ignore_index=args.mask_ignore)
            if gt is not None:
                gt_vis = _draw_class_contours(gt_vis, gt, palette, thickness=2, ignore_index=args.mask_ignore)

        left   = _add_title(img,         'Input')
        middle = _add_title(pseudo_vis,  'Pseudo Label' + (' (overlay)' if args.vis_mode=='overlay' else ''))
        right  = _add_title(gt_vis,      'Ground Truth' + (' (overlay)' if args.vis_mode=='overlay' else ''))
        trip = cv2.hconcat([left, middle, right])
